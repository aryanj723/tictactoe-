...
void printBoard1(char arr[][SIZE]){
...
bool isWin1(char arr[][SIZE], char ch){
    int count;
    for (int i = 0; i < SIZE; i++){
        count = 0;
        for (int j = 0; j < SIZE; j++){
...
int main()
{
    cout<<"Welcome to TIC TAC TOE .This game has 2 modes"<<endl;
    cout<<"Press 1 for personal challenge against me and input anything else for multiplayer mode."<<endl;
    char cho;cin>>cho;
    if(cho=='1')
    {int i,j;
    for(i=0;i<3;i++)
        for(j=0;j<3;j++)
            a[i][j]=0;
input:
    cout<<"Wanna play first or second?press 1 for first 2 for second"<<endl;
    cin>>t;
    switch(t)
    {
        case 1:
    ...
    while (!isGameEnd){
        ...
        if (isWin1(board, player)){
            system("cls");
            printBoard1(board);
            cout << "Player " << player << " Wins" << endl;
            isGameEnd = true;
        }
        else if (isDraw1(board)){
            system("cls");
            printBoard1(board);
            cout << "Game Draw" << endl;
            isGameEnd = true;
